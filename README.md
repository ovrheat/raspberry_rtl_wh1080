...
[RaspberryPi] (https://www.raspberrypi.org/)'s all-in-one rtl-sdr solution to decode [Fine Offset WH1080 weather station] (http://www.foshk.com/weather_professional/wh1080.htm) RF data signal, and to read data from a [BMP085] (https://www.google.com/search?q=BMP085)/[BMP180] (https://www.google.com/search?q=BMP180) barometric sensor using an 'hacked' [rtl_433] (https://github.com/merbanan/rtl_433) snapshot.
------------------------------
...
Hacking the C code has been necessary so the program was passing its values 'on-the-fly' to the Python script instead of writing data to a file. But this approach, summed up to the frequency problems, somehow introduced new instability behaviours...
...
- Get the exact time and date (DCF77 time system and maybe more) coming from the station. By using some scripting you can easily set the Rasp internal clock to the **exact** time and date without the need of NTP or RTC. No data connection required!
