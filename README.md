...
It's composed by an indoor touchscreen control panel and an outdoor wireless sensors group. The latter sends periodically data packets to the indoor console, containing weather measurements about wind speed, wind direction, temperature, humidity and rain. It periodically sends also time signals (DCF77 and maybe other time signals standards) to keep the console clock perfectly synced with an atomic clock.
