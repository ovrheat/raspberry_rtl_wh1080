...
- being a MUCH more stable solution, there's no need to pass data to a datalogger script by using such a 'data file transfer' method. You can grab json data 'on the fly' from the program's output. As a result, the Rasp SDcard is much less stressed and its theorycal life is way longer.  
